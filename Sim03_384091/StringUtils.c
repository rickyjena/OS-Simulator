...
Precondition: given C-style with null characters at end
...
Preconditon: given C-style source string. having a null character ('\0')
             at end of string; destination string is passed in
             as a parameter with enough memory to accept the source string
Postconditon: destination string contains an exact copy
...
void copyString( char *destination, char *source )
   {
    ...
    // loop until null charachter is found in source string
...
void concatenateString( char *destination, char *source )
   {
    ...
    while( sourceIndex < MAX_STR_LEN && source[ sourceIndex ] != NULL_CHAR )
       {
        // assign source charachter to destination at destination index
...
Algorithm: compares two string alphabetically such that:
           ...
           the difference in lengths will be eturned
...
void getSubString( char *destStr, char *sourceStr,
                                       int startIndex, int endIndex )
   {
    // initialize function/variables
       ...
       // intitialize source index to start index parameter
...
Precondition: given a C-style test string, having a null charachter ('\0')
              at end of string, add given search string with
...
Precondition: given a C-style source string, having a null character ('\0')
              at end of string, and destruction string parameter
              is passed with enough memory to hold the lower case string
Postcondition: all upper case letters in given string are sert
...
Precondition: file is open with valid pointer;
